...
function aliasing(accel::MPIAcceleration, x::Chunk, T)
    ...
    # Single broadcast instead of separate bcast calls
    return MPI.bcast(ainfo, x.handle.rank, x.handle.comm)
end
...
function move(from_proc::Processor, to_proc::Processor, x::MPIRef)
    if x.innerRef === nothing
        return nothing  # Or handle this case appropriately
    end
    return move(from_proc, to_proc, poolget(x.innerRef))
end
...
function distribute(A::Union{AbstractArray{T,N}, Nothing}, dist::Blocks{N}; comm::MPI.Comm, root::Int) where {T,N}
    ...
    if isroot
        metadata = (domain(A), eltype(A), typeof(A), partition(dist, domain(A)))
    else
        metadata = nothing
    end
    
    d, type, AT, sd = MPI.bcast(metadata, root, comm)
    
    num_chunks = prod(size(sd))
    if num_chunks < csz
        @warn "Number of chunks is less than number of ranks, performance may be suboptimal"
    end

    # Only allocate array of references
...
function Base.collect(x::Dagger.DArray{T,3}; 
    ...
    if !acrossranks
        ...
        # Determine target rank and ensure all processes know it
        target_rank = isnothing(root) ? 0 : root
        
        if rank == target_rank
            result = Array{T}(undef, size(x))
        else
            result = nothing
        end
        ...
        send_buffers = []
        recv_buffers = Dict{Int, Tuple{Array{T}, NTuple{3,UnitRange{Int}}}}()

        # Process chunks in order
        for (i, chunk) in enumerate(x.chunks)
            chunk_rank = (i - 1) % csz
            subdomain = sd[i]
            shape = map(length, subdomain.indexes)

            if chunk_rank == rank  # This is my chunk
                local_data = fetch(chunk)
                
                if rank == target_rank
                    # Direct copy if we're the target
                    result[subdomain.indexes...] = local_data
                else
                    # Non-blocking send to target rank
                    buffer = copy(local_data) 
                    req = MPI.Isend(buffer, target_rank, i, comm)
                    push!(send_requests, req)
                    push!(send_buffers, buffer)  # Keep buffer alive
                end
            elseif rank == target_rank
                # Non-blocking receive from chunk owner
                recv_buffer = Array{T}(undef, shape...)
                req = MPI.Irecv!(recv_buffer, chunk_rank, i, comm)
                push!(recv_requests, req)
                recv_buffers[i] = (recv_buffer, subdomain.indexes)
            end
        end
        ...
        if !isempty(recv_requests)
            ...
            if rank == target_rank
                for (i, (buffer, indices)) in recv_buffers
