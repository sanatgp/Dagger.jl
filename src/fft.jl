# DaggerFFTs.jl - CPU-based distributed FFT with optimized communication
module DaggerFFTs
...
# ============================================================================
# MEMORY OPERATIONS (Multi-threaded)
# ============================================================================
...
function flat_redistribute!(dst::DArray{T,3}, src::DArray{T,3}, workspace::FFTWorkspace{T}, phase_id::Int) where T
    ...
    # Perform local copies while communication is ongoing
    ...
    # Progressive unpacking: unpack as soon as each receive completes
...
function hierarchical_redistribute!(dst::DArray{T,3}, src::DArray{T,3}, workspace::FFTWorkspace{T}, phase_id::Int) where T
    ...
    # === Phase 1: Intra-node aggregation ===
    # Non-aggregator ranks pack data for their node's aggregator
    ...
    # === Phase 2: Inter-node communication (aggregators only) ===
    if hierarchical.is_aggregator
        ...
        # Wait for inter-node receives and unpack progressively
        ...
        # Wait for inter-node sends
        for i in 1:length(workspace.target_nodes)
            MPI.Wait!(workspace.node_send_reqs[i])
        end
    end
    
    # Synchronize after inter-node communication
    MPI.Barrier(hierarchical.node_comm)
    
    # === Phase 3: Intra-node peer-to-peer communication ===
    # Post receives
    for (i, src_rank) in enumerate(workspace.recv_ranks)
        peer_info = workspace.recv_peers[src_rank]
        tag = generate_tag(src_rank, phase_id)
...
# ============================================================================
# PENCIL DECOMPOSITION FFT (2 transposes)
# ============================================================================
...
# ============================================================================
# SLAB DECOMPOSITION FFT (1 transpose)
# ============================================================================
